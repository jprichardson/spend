...
function simpleTx (fromWIF, toAddress, amountSatoshis, changeAddress, callback) {
  var key = CoinKey.fromWif(fromWIF)
  var blockchain = simpleSend.blockchain
  assert(blockchain, 'simpleSend.blockchain not set. Did you forget to?')
...
function simpleSend (/* arguments, see simpleTx */) {
  ...
  args[args.length - 1] = function (err, rawTx) {
    if (err) return callback(err)
    simpleSend.blockchain.propagate(rawTx, callback)
  }

  simpleTx.apply(simpleSend, args)
}

simpleSend.blockchain = null
simpleSend.simpleTx = simpleTx

module.exports = simpleSend
